  ...
  implements IViewportService, IViewportServiceInternals
{
  ...
  public setBlendMode(viewportId: string, blendMode: string, volumeId?: string): void {
    ...
    if (!isVolumeViewportType(viewport)) {
      console.warn(
        'CornerstoneViewportService: setBlendMode failed - viewport is not a volume viewport',
        viewport?.type
      );
      return;
    }
    ...
    const filterActorUIDs = volumeId ? [volumeId] : undefined;
    viewport.setBlendMode(csBlendMode, filterActorUIDs);
    ...
    if (isProjectionMode) {
      const currentThickness = viewport.getSlabThickness();
      if (!currentThickness || currentThickness < 1) {
        viewport.setSlabThickness(20, filterActorUIDs);
      }
    } else if (csBlendMode === BlendModes.COMPOSITE) {
      viewport.setSlabThickness(0.1, filterActorUIDs);
    }
    ...
    if (viewportInfo) {
      ...
      if (displaySetOptions?.length > 0) {
        ...
        displaySetOptions[0].slabThickness = viewport.getSlabThickness();
        viewportInfo.setDisplaySetOptions(displaySetOptions);
      }
    }
