...
export function useViewportRendering(
  ...
  useEffect(() => {
    ...
    const updateRenderingProps = () => {
      // Always get a fresh viewport: the closed-over reference is stale after a
      // stack -> volume transition
      const currentViewport = cornerstoneViewportService.getCornerstoneViewport(viewportId);
      if (!currentViewport) {
        return;
      }

      if (isVolumeViewportType(currentViewport)) {
        const volumeIds = currentViewport.getAllVolumeIds();
        const volumeId =
          volumeIds.find(id => id.includes(activeDisplaySetInstanceUID)) ?? volumeIds[0];
        setBlendModeState(currentViewport.getBlendMode(volumeId ? [volumeId] : undefined));
        setSlabThicknessState(currentViewport.getSlabThickness());
      } else {
